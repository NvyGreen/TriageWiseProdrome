...
Notes:
  ...
  - The queue is the persisted triage_queue table; a POST inserts a row (via the
    scorer) and a PATCH reprioritizes it.
  - The clinical case seeds a filler ahead of the target so the re-score produces
    a real move (reprioritized fires only on movement). The filler is seeded at
...
def _seed_queued(db_session, chief_complaint, esi_band, arrival, **vitals):
...
def test_submit_logs_expected_events(client, db_session, name):
    case = BY_NAME[name]
...
def test_unscoreable_submit_logs_nothing(client, db_session):
...
def test_every_row_has_event_type_and_ids(client, db_session):
...
def test_clinical_update_logs_events(client, db_session):
    case = BY_NAME["clinical_update_logs_case_updated_and_scored"]

    # Filler ahead (earlier arrival), target behind; both ESI-3.
    filler = _seed_queued(db_session, "abdominal", 3, _at("09:00"))
    target = _seed_queued(
        db_session, "respiratory", 3, _at("10:00"),
...
def test_status_only_update_logs_status_changed(client, db_session):
    case = BY_NAME["status_only_update_logs_status_changed_only"]
    intake_id = _seed_queued(db_session, "cardiac", 2, _at("10:00"))
...
def test_disposition_logs_status_changed(client, db_session):
